"""
Architecture Comparator: UNETR (Reviewer R2.2)
====================================================
Trains a UNETR-based regressor on OASIS-1 under the IDENTICAL protocol
...
UNETR is a segmentation network; for scalar nWBV regression we use it as
...
Output: experiments/arch_comparator_unetr/results.json
"""
...
PATIENCE     = 10
SEED         = 42
OUT_DIR      = project_root / "experiments" / "arch_comparator_unetr"


class UNETRRegressor(nn.Module):
    """UNETR feature extractor -> GAP -> scalar nWBV."""
    def __init__(self, feature_size: int = 16):
        super().__init__()
        self.backbone = UNETR(in_channels=1, out_channels=8,
                              img_size=(64, 64, 64),
...
def main():
    ...
    print("=" * 62)
    print("UNETR Architecture Comparator (OASIS, R2.2)")
    print("=" * 62, flush=True)
    ...
    model = UNETRRegressor().to(DEVICE)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"UNETR regressor: {n_params:,} params", flush=True)
    ...
    print("\n" + "=" * 62)
    print(f"UNETR OASIS TEST:  MAE={mae:.4f}  Pearson r={r_test:.3f}  "
          f"(n_test={len(trues)}, params={n_params:,})")
    print("=" * 62, flush=True)

    out = {
        "experiment":  "arch_comparator_unetr",
        "addresses":   ["R2.2"],
        "model":       "UNETR (feature_size=48) + GAP + linear head",
        "params":      n_params,
